...
parser.add_argument('--attr_ratio', type=float, default=1, help='')
parser.add_argument('--topo_ratio', type=float, default=1, help='')
...
def gae_for(args):
    ...
    tmp_adj = np.array(adj.todense()).astype(np.int16)
    graph = nx.random_graphs.gnp_random_graph(n_nodes, args.topo_ratio)
    graph = nx.to_numpy_array(graph).astype(np.int16)
    shuf_adj = np.bitwise_xor(tmp_adj, graph)
    shuf_adj = sp.csc_matrix(shuf_adj)
    ...
    shuf_adj_norm = preprocess_graph(shuf_adj)
    shuf_adj_label = shuf_adj + sp.eye(shuf_adj.shape[0])
    # adj_label = sparse_to_tuple(adj_label)
    shuf_adj_label = torch.FloatTensor(shuf_adj_label.toarray())

    pos_weight = float(adj.shape[0] * adj.shape[0] - adj.sum()) / adj.sum()
    norm = adj.shape[0] * adj.shape[0] / float((adj.shape[0] * adj.shape[0] - adj.sum()) * 2)

    shuf_pos_weight = float(shuf_adj.shape[0] * shuf_adj.shape[0] - shuf_adj.sum()) / shuf_adj.sum()
    shuf_norm = shuf_adj.shape[0] * shuf_adj.shape[0] / float((shuf_adj.shape[0] * shuf_adj.shape[0] - shuf_adj.sum()) * 2)
    ...
    for epoch in range(args.epochs):
        ...
        fakeadj, fakez = below(features, shuf_adj_norm)
        ...
        re, z = below(features, shuf_adj_norm)
        below_loss = D(features, z)
        recovered, mu = model(features, adj_norm)
        loss = loss_function(preds=recovered, labels=adj_label, norm=norm, pos_weight=pos_weight)
        below_reloss = loss_function(preds=re, labels=shuf_adj_label, norm=shuf_norm, pos_weight=shuf_pos_weight)
        D_result = D(features, mu)
        D_real_loss= D_result
        ...
        optimizer.step()
        below_mi = torch.mean(below_M(features, z)) - torch.log(torch.mean(torch.exp(below_M(features, mu))) + 1e-8)
        ...
        learn_mine(below, below_M, below_M_opt, features, shuf_adj_norm, shuf_fts)
        ##################
...
if __name__ == '__main__':
    ...
    for i in [0.001, 0.005, 0.01, 0.05, 0.1, 0.2, 0.4, 0.6, 0.8]:
        args.topo_ratio = i
        gae_for(args)
    '''
