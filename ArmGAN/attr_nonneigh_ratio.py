...
def gae_for(args):
    ...
    for epoch in range(args.epochs):
        ...
        shuf_fts = features.clone()
        idy = idx[:int(n_nodes * args.attr_ratio)]
        #print(np.where(adj_tmp[1] == 1)[1])
        for i in idy:
            one_entry = np.where(adj_tmp[i] == 0)[1]
            if len(one_entry) == 0:
                continue
            id_one = np.random.permutation(len(one_entry))[0]
            shuf_fts[[i, one_entry[id_one]]] = shuf_fts[[one_entry[id_one], i]]
